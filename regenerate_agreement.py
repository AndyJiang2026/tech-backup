...
def create_agreement():
    ...
    items_1 = [
        '1、甲方应及时完整地向乙方提供必要的相关基础材料，保证所提交的材料及文件内容真实、合法且有效，并依法承担由此产生的全部法律责任。',
        '2、配合乙方做好与相关第三方的沟通事宜，协助乙方办理双方认为其他必要的事宜。',
        '3、甲方应支付乙方咨询顾问费，在材料申报过程中，如有涉及财务审计、专利或著作权申请、用户报告、查新等第三方的所有费用，应由甲方自行委托并独立承担，与应支付乙方的咨询顾问费用无关。',
        '4、甲方指定的项目联系人，配合乙方开展相关工作、提供资料等，如甲方更换项目联系人，应当及时通知乙方。',
        '5、乙方为甲方上述资金申请的唯一合作人，甲方须严格为乙方的服务内容及双方合作事宜保密。'
    ]
    
    for i, item in enumerate(items_1):
        p = doc.add_paragraph()
        add_run(p, item, 'black')
        if i == 4:  # 第 5 条添加修改建议
    ...
    items_2 = [
        '1、在甲方提供真实、完整且与公司实际经营情况一致的基础数据及材料的前提下，乙方为甲方申报上述项目提供咨询服务。',
        '2、乙方应尽职完成本合同约定的咨询服务内容，维护甲方利益。',
        '3、甲方有权就乙方服务范围内的事项，向乙方提出口头或书面询问，乙方应及时作出答复。',
        '4、由于项目申报在流程上有一定的阶段性，在每个阶段的工作中，乙方有责任配合甲方完成申报工作，向申报部门提交全部申报材料后即视为乙方已完成全部咨询服务内容，如在合同履行过程中甲方对乙方的工作表示不满，应及时向乙方提出，乙方应及时作出解释或整改。但甲方不得在成功申请到政府资助金后，又以乙方履行瑕疵为由提出拒付或少付咨询顾问费。',
        '5、乙方有义务严格遵守顾问服务的行业标准，保护甲方的合法权益，严守甲方的机密，严格为双方合作事宜保密。'
    ]
    ...
    items_4 = [
        '1、如果乙方工作人员在整理甲方的申报材料时发现甲方项目和企业条件有严重不足而无法申报本合同规定的项目时，双方可协商申报另外的科技计划或专项资金项目。若因自然灾害、政府政策调整等不可抗力导致合同无法履行，双方可延期或终止合同，不承担违约责任。',
        '2、合同签订生效后，如乙方无正当理由，擅自中止对甲方的服务，则应赔偿由此造成的损失；如甲方单方面停止履行合同，并自行进行申报，视同本合同仍然生效，仍须按上述支付费用条款支付乙方的咨询顾问费。',
        '3、成功申请到政府资助金后，如甲方拒绝接受政府资助，仍视同乙方已完成合同服务内容，甲方仍须支付上述咨询顾问费，按照同批其他企业的最高立项金额在第一批拨款后的十个工作日内，一次性付清。',
        '4、如甲方逾期付款，乙方将向甲方另行加收利息，利息按应付款额每日万分之八计算，直至付清。',
        '5、甲方每收到一笔本项目的政府资助金后，应及时告知乙方（不超过 3 个工作日）收到款项的日期和数额，如果乙方向甲方询问关于本项目政府资助金回款情况，甲方拒绝告知的，则乙方有权随时要求甲方按原始申请资金额度支付本项目全部的咨询顾问费，甲方不予支付的，视为甲方逾期付款，应承担相应违约责任。'
    ]
    
    suggestions_4 = [
        None,
        '【修改建议：增加"如乙方未按约定提供服务或服务质量严重不达标，甲方有权解除合同并要求退还已支付费用"】',
        '【修改建议：删除此条款，或改为"如甲方无正当理由拒绝接受资助，应按实际获批金额的 50% 支付咨询费"】',
        '【修改建议：改为"万分之三"，万分之八年化约 29%，过高】',
        '【修改建议：删除"按原始申请资金额度支付全部咨询费"条款，改为"甲方应如实告知，如故意隐瞒，乙方有权通过合法途径查询并追究违约责任"】'
    ]
    ...
    items_5 = [
        '1、双方因履行本协议发生的争议，应首先通过友好协商解决，协商不成时，任何一方可向乙方所在地人民法院提起诉讼解决。',
        '2、本合同执行期为自合同签订之日起，三年内有效（付款时间不受此条款限制）。',
        '3、本合同一式两份，双方各持一份，本合同经双方盖章后立即生效。'
    ]
    
    suggestions_5 = [
        '【修改建议：改为"甲方所在地"或"合同签订地"人民法院，或约定仲裁】',
        '【修改建议：改为"至项目申报服务完成之日止"，或明确"有效期两年"】',
        None
    ]
...
if __name__ == '__main__':
    doc = create_agreement()
    output_path = '/home/admin/.openclaw/workspace-chief-agent/业务约定书（审查修改版）.docx'
    doc.save(output_path)
    print('✅ 文档已重新生成：' + output_path)
    print('文件大小：约 40KB')
    print('格式：Microsoft Word (.docx)')
