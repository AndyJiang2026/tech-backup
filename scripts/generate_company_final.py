#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""生成 XR 项目立项报告 Word 文档（科技公司呈报版·最终润色）"""
...
def create_report():
    ...
    run.font.bold = True
    run.font.size = Pt(12)
    run = p.add_run('助力贵台落实"组建专业 XR 技术团队""盘活存量资产""推进技术资源变现"要求，')
    run.font.size = Pt(12)
    run = p.add_run('向"技术+"要空间，')
    run.font.bold = True
    run.font.size = Pt(12)
    run = p.add_run('为贵台新闻宣传、媒体融合、经营发展三大提质增效行动贡献力量。')
    run.font.size = Pt(12)
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    p.paragraph_format.space_after = Pt(12)
    ...
    # 保存文档
    doc.save('reports/XR 项目立项报告（科技公司呈报版·最终润色）.docx')
    print('✅ 科技公司呈报版（最终润色）Word 文档生成成功！')
    print('文件路径：/home/admin/.openclaw/workspace-chief-agent/reports/XR 项目立项报告（科技公司呈报版·最终润色）.docx')
