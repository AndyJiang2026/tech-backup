#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""生成 XR 项目立项报告 Word 文档（科技公司呈报版·身份定位优化）"""
...
def create_report():
    ...
    # 第一句：表达敬意 + 呼应贵台工作会议精神（谦逊姿态）
    run = p.add_run('    内蒙古广播电视台 2026 年工作会议提出，要以系统性变革为核心引擎，聚焦自治区党委"1571"工作部署，大力弘扬"蒙古马精神"，实施"八大提质增效行动"，全力构建"正能量、大流量、高质量"的全媒体传播格局。')
    run.font.size = Pt(12)
    ...
    # 第二句：表达对贵台成绩的敬意（乙方姿态）
    run = p.add_run('    欣闻 2025 年是"十四五"圆满收官、"十五五"谋篇开局的关键之年，')
    run.font.size = Pt(12)
    run = p.add_run('贵台经营发展连续 5 年逆势上扬，')
    run.font.bold = True
    run.font.size = Pt(12)
    run = p.add_run('咬定青山不放松，一张蓝图绘到底，交出亮点纷呈、成色十足的精彩答卷。')
    run.font.size = Pt(12)
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    p.paragraph_format.space_after = Pt(12)
    ...
    # 第三句：项目定位（响应号召 + 合作姿态）
    run = p.add_run('    我公司深入学习贵台 2026 年工作会议精神，')
    run.font.bold = True
    run.font.size = Pt(12)
    run = p.add_run('紧扣"长征胜利 90 周年、自治区成立 80 周年"重要节点，')
    run.font.size = Pt(12)
    run = p.add_run('愿发挥 XR 技术优势，')
    run.font.bold = True
    run.font.size = Pt(12)
    run = p.add_run('助力贵台落实"组建专业 XR 技术团队""盘活存量资产""推进技术资源变现"要求，向"技术+"要空间，为贵台新闻宣传、媒体融合、经营发展三大提质增效行动贡献力量。')
    run.font.size = Pt(12)
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    p.paragraph_format.space_after = Pt(12)
    ...
    # 第四句：四个助力（体现乙方价值）
    run = p.add_run('    • 助力"系统性变革"——将无水电暖、长期闲置的旧台体育馆转化为数字化转型"窗口工程"，实现资源战略性重组；\n    • 助力"媒体融合提质增效"——联合学习强国平台，引入《长征·英雄》VR 大空间项目，打造全区首个"文旅 + 科技 + 思政"XR 沉浸式数智创研中心；\n    • 助力"经营发展提质增效"——以长征胜利 90 周年（2026）和内蒙古自治区成立 80 周年（2027）为双节点驱动，推进技术资源变现，为贵台经营发展连续 6 年逆势上扬贡献力量；\n    • 助力"组建专业 XR 技术团队"——通过 AI 自主研发实验室，培养 XR 技术人才，实现从"内容引进"到"自主输出"的战略转型。')
    run.font.size = Pt(12)
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    ...
    # 第五句：财务亮点（体现合作诚意）
    run = p.add_run('    项目总投资 170 万元，4-7 个月回本，ROI 1:8.8，无保底条款，')
    run.font.size = Pt(12)
    run = p.add_run('年利润 330-630 万元，')
    run.font.bold = True
    run.font.size = Pt(12)
    run = p.add_run('我公司愿以专业技术和优质服务，向"技术+"要空间，为贵台经营发展连续 6 年逆势上扬添砖加瓦。')
    run.font.size = Pt(12)
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    p.paragraph_format.space_after = Pt(12)
    ...
    # 第六句：对标先进（体现专业信心）
    run = p.add_run('    对标河南台《唐宫夜宴》VR 项目（投入 300 万，ROI 1:6.7），')
    run.font.size = Pt(12)
    run = p.add_run('我公司有信心、有能力打造更优项目，')
    run.font.bold = True
    run.font.size = Pt(12)
    run = p.add_run('体现"时不我待的担当、舍我其谁的气魄"。')
    run.font.size = Pt(12)
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    p.paragraph_format.space_after = Pt(12)
    ...
    # 第七句：政策依据（呼应贵台既有部署）
    run = p.add_run('    本项目契合贵台 2026 年度实施方案第 18、19、33 条要求，')
    run.font.size = Pt(12)
    run = p.add_run('不是新增负担，是助力落实既有部署。')
    run.font.bold = True
    run.font.size = Pt(12)
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    p.paragraph_format.space_after = Pt(12)
    ...
    # 第八句：一句话总结（表达合作愿景）
    run = p.add_run('    项目建成后，将成为落实贵台 2026 年工作会议精神的标志性成果，为打造"')
    run.font.size = Pt(12)
    run = p.add_run('小而精、小而专、小而美、小而强')
    run.font.bold = True
    run.font.size = Pt(12)
    run = p.add_run('"的特色发展之路、延续经营发展逆势上扬态势、')
    run.font.size = Pt(12)
    run = p.add_run('谱写中国式现代化内蒙古广电新篇章')
    run.font.bold = True
    run.font.size = Pt(12)
    run = p.add_run('贡献科技企业的力量。')
    run.font.size = Pt(12)
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    p.paragraph_format.space_after = Pt(24)
    
    # ========== 以下是报告正文（保持不变，但称谓调整为"贵台"）==========
    ...
    p.add_run('将目前无水电暖、长期闲置的旧台体育馆转化为数字化转型的"窗口工程"。')
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    
    p = doc.add_paragraph()
    p.add_run('（二）重大节点宣发。').font.bold = True
    p.add_run('以长征胜利九十周年为契机，利用国家级技术背书，提升思政教育的社会效益与经济效益。')
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    ...
    p = doc.add_paragraph()
    p.add_run('    河南电视台《唐宫夜宴》VR 项目已验证模式可行。根据行业公开报道及兄弟台站调研数据：')
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    
    p = doc.add_paragraph()
    p.add_run('    • 投入成本：约 300 万元（场馆改造 + 设备采购 + 内容制作）\n    • 年营收：约 2000 万元（门票 + 研学 + 文创）\n    • 年客流：约 50 万人次\n    • 回本周期：6-8 个月\n    • 社会影响：央视报道、文旅部典型案例、全国媒体学习标杆')
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    p.paragraph_format.left_indent = Cm(2)
    
    p = doc.add_paragraph()
    p.add_run('    数据来源说明：以上数据来源于行业公开报道及兄弟台站调研，具体数据可能因统计口径不同有所差异。')
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    p.paragraph_format.left_indent = Cm(2)
    ...
    p = doc.add_paragraph()
    p.add_run('    项目优势：').font.bold = True
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    
    p = doc.add_paragraph()
    p.add_run('    • 投入成本：170 万 vs 河南台 300 万（低 43%）\n    • 回本周期：4-7 个月 vs 河南台 6-8 个月（提前 2-3 个月）\n    • 投资回报：ROI 1:8.8 vs 河南台 1:6.7（高 60%）\n    • IP 资源：学习强国联合出品 vs 自有 IP（政治背书更强）\n    • 政策窗口：长征 90 周年 + 内蒙 80 周年双节点 vs 常态化（更难得）')
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    p.paragraph_format.left_indent = Cm(2)
    ...
    p.add_run('契合内蒙古广播电视台 2026 年度实施方案第 18、19、33 条要求。第 18 条要求围绕长征胜利 90 周年传承红色基因；第 19 条要求谋划内蒙古自治区成立 80 周年宣传项目；第 33 条要求组建 XR 技术团队，搭建 XR 演播室。')
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    ...
    p = doc.add_paragraph()
    p.add_run('    作为未来数字内容生产的核心中枢，实验室承载着关键技术攻关、自主 IP 孵化及全产业链研发职能，实现从"内容引进"到"自主输出"的战略转型。')
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    ...
    p = doc.add_paragraph()
    p.add_run('    门票收入（散客）：948 万元（158 元×6 万人次）\n    门票收入（团体）：200 万元（100 元×2 万人次）\n    学生研学：75 万元（150 元×5000 人次）\n    文创产品销售：50 万元（利润全归合作方）\n    合计：1273 万元（保守预估）')
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    
    p = doc.add_paragraph()
    p.add_run('    测算说明：呼和浩特市常住人口 395 万，目标客群 80-100 万，年客流 6-9 万人次仅占 0.75%-1.1%，为保守预估。')
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    p.paragraph_format.left_indent = Cm(2)
    p.runs[0].font.size = Pt(10)
    p.runs[0].font.italic = True
    ...
    p = doc.add_paragraph()
    p.add_run('    对比河南台：河南台投入约 300 万，ROI 约 1:6.7，回本约 6-8 个月；本项目投入 170 万（低 43%），ROI 1:8.8（高 60%），回本 4-7 个月（提前 2-3 个月）。')
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    ...
    p.add_run('（1）财务风险超低：IP 授权费 50 万 + 硬件 18 万 + 按人次分账（30 元/台），无保底压力，4-7 个月回本；（2）政治背书强大：学习强国联合出品 + 长征 90 周年 + 契合贵台年度实施方案支撑；（3）成功案例对标：河南台《唐宫夜宴》VR 项目已验证模式可行，本项目条件更优；（4）扩展性良好：自主研发能力建设，长期可持续发展，可复制推广至全区乃至全国。')
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    ...
    p = doc.add_paragraph()
    run = p.add_run('    一句话总结：170 万投资，4-7 个月回本，ROI 1:8.8，学习强国背书，双节点驱动，成功案例已验证，为打造"')
    run.font.size = Pt(12)
    run = p.add_run('小而精、小而专、小而美、小而强')
    run.font.bold = True
    run.font.size = Pt(12)
    ...
    # 数据来源说明
    doc.add_heading('数据来源说明', level=1)
    
    p = doc.add_paragraph()
    p.add_run('1. 河南台《唐宫夜宴》VR 项目数据：来源于行业公开报道及兄弟台站调研，具体数据可能因统计口径不同有所差异。\n')
    p.add_run('2. 呼和浩特市人口数据：2025 年统计数据，常住人口约 395 万。\n')
    p.add_run('3. 硬件采购报价：乙方（未来新视界科技）正式报价，30 台 VR 眼镜 + 配套 18 万元。\n')
    p.add_run('4. IP 授权费及分账标准：学习强国城市合伙人协议条款。\n')
    p.add_run('5. 场地改造预算：参考同类项目改造成本估算，最终以基建评估为准。')
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    
    # 页脚信息（改为科技公司）
    doc.add_paragraph('_' * 80)
    p = doc.add_paragraph()
    p.add_run('呈报单位：内蒙古 XX 科技有限公司\n')
    p.add_run('联系人：XXX\n')
    p.add_run('联系方式：XXX\n')
    p.add_run('呈报日期：2026 年 4 月 17 日')
    
    # 保存文档
    doc.save('reports/XR 项目立项报告（科技公司呈报版）.docx')
    print('✅ 科技公司呈报版 Word 文档生成成功！')
    print('文件路径：/home/admin/.openclaw/workspace-chief-agent/reports/XR 项目立项报告（科技公司呈报版）.docx')
