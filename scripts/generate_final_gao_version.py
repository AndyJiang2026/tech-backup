#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""生成 XR 项目立项报告 Word 文档（最终版·深度融入高台长讲话）"""
...
def create_report():
    ...
    run.font.bold = True
    run.font.size = Pt(12)
    run = p.add_run('实施新闻宣传提质增效行动和媒体融合提质增效行动。')
    run.font.size = Pt(12)
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    p.paragraph_format.space_after = Pt(12)
    ...
    run.font.size = Pt(12)
    run = p.add_run('为经营发展连续 6 年逆势上扬贡献力量。')
    run.font.bold = True
    run.font.size = Pt(12)
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    p.paragraph_format.space_after = Pt(12)
    ...
    # 保存文档
    doc.save('reports/XR 项目立项报告（最终版·深度融入讲话）.docx')
    print('✅ 最终版（深度融入讲话）Word 文档生成成功！')
    print('文件路径：/home/admin/.openclaw/workspace-chief-agent/reports/XR 项目立项报告（最终版·深度融入讲话）.docx')
