#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""生成 XR 项目立项报告 Word 文档（高台长审阅版·融入讲话原文）"""
...
def create_report():
    ...
    # 第一句：呼应 2025 年成绩（经营发展连续 5 年逆势上扬）
    run = p.add_run('    全台 2026 年工作会议指出，2025 年是"十四五"圆满收官、"十五五"谋篇开局的关键之年，我台经营发展连续 5 年逆势上扬，走出"量质齐升、全域突破"的黄金五年。')
    run.font.size = Pt(12)
    ...
    # 第二句：呼应发展路径（小而精、小而专、小而美、小而强）
    run = p.add_run('    会议提出，要持续探索西部民族地区省级广电"小而精、小而专、小而美、小而强"的特色发展之路，')
    run.font.size = Pt(12)
    run = p.add_run('一体推进改革创新，')
    run.font.bold = True
    run.font.size = Pt(12)
    run = p.add_run('实现广电媒体系统性变革。')
    run.font.size = Pt(12)
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    p.paragraph_format.space_after = Pt(12)
    ...
    # 第三句：项目定位（呼应系统性变革 + 媒体融合）
    run = p.add_run('    本项目是落实全台 2026 年工作会议精神的具体行动：')
    run.font.bold = True
    run.font.size = Pt(12)
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    p.paragraph_format.space_after = Pt(12)
    ...
    # 第四句：三个呼应
    run = p.add_run('    • 呼应"系统性变革"要求——将无水电暖、长期闲置的旧台体育馆转化为数字化转型"窗口工程"，破茧成蝶；\n    • 呼应"媒体融合变革"要求——联合学习强国平台，引入《长征·英雄》VR 大空间项目，打造全区首个"文旅 + 科技 + 思政"XR 沉浸式数智创研中心；\n    • 呼应"经营发展变革"要求——以长征胜利 90 周年（2026）和内蒙古自治区成立 80 周年（2027）为双节点驱动，')
    run.font.size = Pt(12)
    ...
    run.font.size = Pt(12)
    run = p.add_run('为打造"小而精、小而专、小而美、小而强"的特色发展之路贡献融媒体力量。')
    run.font.bold = True
    run.font.size = Pt(12)
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    ...
    # 保存文档
    doc.save('reports/XR 项目立项报告（高台长审阅版·讲话融入）.docx')
    print('✅ 高台长审阅版（讲话融入）Word 文档生成成功！')
    print('文件路径：/home/admin/.openclaw/workspace-chief-agent/reports/XR 项目立项报告（高台长审阅版·讲话融入）.docx')
