#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""生成 XR 项目立项报告 Word 文档（高台长审阅版·融入 2026 年工作会议精神）"""
...
def create_report():
    ...
    # ========== 优化后的引言（融入高台长 2026 年工作会议精神）==========
    p = doc.add_paragraph()
    
    # 第一句：紧扣 2026 年工作会议主题（策马奔腾启新程 奋楫争先谱新篇）
    run = p.add_run('    全台 2026 年工作会议提出"策马奔腾启新程 奋楫争先谱新篇"，要求深入贯彻落实习近平总书记对内蒙古系列重要讲话重要指示精神，锚定自治区党委"1571"工作部署，全面推进高质量发展。')
    run.font.size = Pt(12)
    ...
    # 第三句：三个落实（对应台长关注的政策落地）
    run = p.add_run('    • 落实"盘活闲置资源"要求——将无水电暖、长期闲置的旧台体育馆转化为数字化转型"窗口工程"；\n    • 落实"媒体融合发展"要求——联合学习强国平台，引入《长征·英雄》VR 大空间项目，打造全区首个"文旅 + 科技 + 思政"XR 沉浸式数智创研中心；\n    • 落实"高质量发展"要求——以长征胜利 90 周年（2026）和内蒙古自治区成立 80 周年（2027）为双节点驱动，实现社会效益与经济效益双丰收。')
    run.font.size = Pt(12)
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    ...
    # 第四句：财务亮点（呼应台长"分析形势"要求）
    run = p.add_run('    项目总投资 170 万元，4-7 个月回本，ROI 1:8.8，无保底条款，财务风险可控。')
    run.font.bold = True
    run.font.size = Pt(12)
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    p.paragraph_format.space_after = Pt(12)
    ...
    # 第七句：一句话总结（呼应"启新程 谱新篇"）
    run = p.add_run('    项目建成后，将成为落实全台 2026 年工作会议精神的标志性成果，')
    run.font.size = Pt(12)
    run = p.add_run('为"策马奔腾启新程 奋楫争先谱新篇"贡献融媒体力量。')
    run.font.bold = True
    run.font.size = Pt(12)
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    ...
    # 保存文档
    doc.save('reports/XR 项目立项报告（高台长审阅版）.docx')
    print('✅ 高台长审阅版 Word 文档生成成功！')
    print('文件路径：/home/admin/.openclaw/workspace-chief-agent/reports/XR 项目立项报告（高台长审阅版）.docx')
