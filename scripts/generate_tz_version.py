#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""生成 XR 项目立项报告 Word 文档（台长审阅版·引言优化）"""
...
def create_report():
    ...
    # 第一句：政策高度 + 战略机遇（台长关心政治站位）
    run = p.add_run('    当前，数字经济与思政教育创新已成为国家战略导向，媒体融合数字化转型进入攻坚期。')
    run.font.size = Pt(12)
    ...
    # 第二句：痛点切入（闲置资产 + 转型压力）
    run = p.add_run('    我台旧台体育馆长期闲置，无水电暖供应，资产沉睡多年；同时，全台数字化转型压力日增，亟需打造标志性"窗口工程"。')
    run.font.size = Pt(12)
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    p.paragraph_format.space_after = Pt(12)
    ...
    # 第三句：解决方案 + 核心优势（学习强国背书 + 双节点）
    run = p.add_run('    本项目联合学习强国平台，引入《长征·英雄》VR 大空间项目，')
    run.font.size = Pt(12)
    run = p.add_run('以长征胜利 90 周年（2026）和内蒙古自治区成立 80 周年（2027）为双节点驱动，')
    run.font.bold = True
    run.font.size = Pt(12)
    run = p.add_run('盘活旧台闲置资产，打造全区首个"文旅 + 科技 + 思政"XR 沉浸式数智创研中心。')
    run.font.size = Pt(12)
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    p.paragraph_format.space_after = Pt(12)
    ...
    # 第五句：对标案例（增强信心）
    run = p.add_run('    河南台《唐宫夜宴》VR 项目已验证模式可行（投入 300 万，ROI 1:6.7），我台条件更优，成功概率更高。')
    run.font.size = Pt(12)
    ...
    # 第七句：一句话总结（打动台长）
    run = p.add_run('    项目建成后，将实现社会效益（全区思政教育示范基地）与经济效益（年利润 330-630 万）双丰收，')
    run.font.size = Pt(12)
    run = p.add_run('成为台长政绩工程、行业标杆项目、上级表彰典型。')
    run.font.bold = True
    run.font.size = Pt(12)
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    ...
    p = doc.add_paragraph()
    run = p.add_run('    一句话总结：170 万投资，4-7 个月回本，ROI 1:8.8，学习强国背书，双节点驱动，成功案例已验证，不做是失职，做了是政绩。')
    run.font.bold = True
    run.font.italic = True
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    p.paragraph_format.space_after = Pt(24)
    ...
    # 保存文档
    doc.save('reports/XR 项目立项报告（台长审阅版）.docx')
    print('✅ 台长审阅版 Word 文档生成成功！')
    print('文件路径：/home/admin/.openclaw/workspace-chief-agent/reports/XR 项目立项报告（台长审阅版）.docx')
