#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""生成 XR 项目立项报告 Word 文档（终极润色版·引言优化）"""
...
def create_report():
    ...
    # 第一句：呼应 2026 年工作部署 + 蒙古马精神
    run = p.add_run('    全台 2026 年工作会议提出，要以系统性变革为核心引擎，聚焦自治区党委"1571"工作部署，大力弘扬"蒙古马精神"，实施"八大提质增效行动"，全力构建"正能量、大流量、高质量"的全媒体传播格局。')
    run.font.size = Pt(12)
    ...
    # 第二句：呼应经营成绩 + 咬定青山不放松
    run = p.add_run('    2025 年是"十四五"圆满收官、"十五五"谋篇开局的关键之年，我台经营发展连续 5 年逆势上扬，')
    run.font.size = Pt(12)
    run = p.add_run('咬定青山不放松，一张蓝图绘到底，')
    run.font.bold = True
    run.font.size = Pt(12)
    run = p.add_run('交出亮点纷呈、成色十足的精彩答卷。')
    run.font.size = Pt(12)
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    p.paragraph_format.space_after = Pt(12)
    ...
    # 第三句：项目定位（融入 XR 技术团队、盘活存量资产、技术资源变现）
    run = p.add_run('    本项目是落实高文鸿台长工作部署的具体行动，紧扣"长征胜利 90 周年、自治区成立 80 周年"重要节点，')
    run.font.size = Pt(12)
    run = p.add_run('落实"组建专业 XR 技术团队""盘活存量资产""推进技术资源变现"要求，')
    run.font.bold = True
    run.font.size = Pt(12)
    run = p.add_run('向"技术+"要空间，')
    run.font.bold = True
    run.font.size = Pt(12)
    run = p.add_run('实施新闻宣传、媒体融合、经营发展三大提质增效行动。')
    run.font.size = Pt(12)
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    p.paragraph_format.space_after = Pt(12)
    ...
    # 第四句：四个落实（精简表达，突出关键词）
    run = p.add_run('    • 落实"系统性变革"要求——将无水电暖、长期闲置的旧台体育馆转化为数字化转型"窗口工程"，实现资源战略性重组；\n    • 落实"媒体融合提质增效"要求——联合学习强国平台，引入《长征·英雄》VR 大空间项目，打造全区首个"文旅 + 科技 + 思政"XR 沉浸式数智创研中心；\n    • 落实"经营发展提质增效"要求——以长征胜利 90 周年（2026）和内蒙古自治区成立 80 周年（2027）为双节点驱动，推进技术资源变现，为经营发展连续 6 年逆势上扬贡献力量；\n    • 落实"组建专业 XR 技术团队"要求——通过 AI 自主研发实验室，培养 XR 技术人才，实现从"内容引进"到"自主输出"的战略转型。')
    run.font.size = Pt(12)
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    ...
    run.font.bold = True
    run.font.size = Pt(12)
    run = p.add_run('向"技术+"要空间，为经营发展连续 6 年逆势上扬贡献力量。')
    run.font.bold = True
    run.font.size = Pt(12)
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    p.paragraph_format.space_after = Pt(12)
    ...
    # 第六句：对标先进（呼应奋楫争先）
    run = p.add_run('    对标河南台《唐宫夜宴》VR 项目（投入 300 万，ROI 1:6.7），我台条件更优，')
    run.font.size = Pt(12)
    ...
    # 第七句：政策依据（呼应咬定青山不放松）
    run = p.add_run('    本项目是落实台内 2026 年度实施方案第 18、19、33 条的具体行动，')
    run.font.size = Pt(12)
    run = p.add_run('咬定青山不放松，')
    run.font.bold = True
    run.font.size = Pt(12)
    run = p.add_run('不是新增项目，是落实既有部署。')
    run.font.size = Pt(12)
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    p.paragraph_format.space_after = Pt(12)
    ...
    # 第八句：一句话总结（呼应小而强 + 贡献广电力量）
    run = p.add_run('    项目建成后，将成为落实全台 2026 年工作会议精神的标志性成果，为打造"')
    run.font.size = Pt(12)
    ...
    p = doc.add_paragraph()
    p.add_run('    数据来源说明：以上数据来源于行业公开报道及兄弟台站调研，具体数据可能因统计口径不同有所差异。我台项目参考其商业模式，结合本地市场实际情况进行保守测算。')
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    p.paragraph_format.left_indent = Cm(2)
    ...
    p = doc.add_paragraph()
    p.add_run('    我台对比优势：').font.bold = True
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    ...
    p = doc.add_paragraph()
    p.add_run('    对比河南台：河南台投入约 300 万，ROI 约 1:6.7，回本约 6-8 个月；我台投入 170 万（低 43%），ROI 1:8.8（高 60%），回本 4-7 个月（提前 2-3 个月）。')
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    ...
    p.add_run('（1）财务风险超低：IP 授权费 50 万 + 硬件 18 万 + 按人次分账（30 元/台），无保底压力，4-7 个月回本；（2）政治背书强大：学习强国联合出品 + 长征 90 周年 + 台内年度实施方案支撑；（3）成功案例对标：河南台《唐宫夜宴》VR 项目已验证模式可行，我台条件更优；（4）扩展性良好：自主研发能力建设，长期可持续发展，可复制推广至全区乃至全国。')
    p.paragraph_format.line_spacing_rule = WD_LINE_SPACING.ONE_POINT_FIVE
    ...
    # 保存文档
    doc.save('reports/XR 项目立项报告（终极润色版·引言优化）.docx')
    print('✅ 终极润色版（引言优化）Word 文档生成成功！')
    print('文件路径：/home/admin/.openclaw/workspace-chief-agent/reports/XR 项目立项报告（终极润色版·引言优化）.docx')
